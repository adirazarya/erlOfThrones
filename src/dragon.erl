...
%% Get walk event while the dragon is walking.
...
%% Get gotoWhiteking event while the dragon is walking.
%% Check if you are stuffed:
%%							TRUE : Continue walking.
%%							FALSE : Change state to walking4Food state.
walking({srv_gotoWhiteking, Wpos}, StateData) ->                    % Dragon is near a whiteking, into whiteking_radius area.
	case get(stuffed) of
...
%% Get stuffedTimeout event while the dragon is walking.
%% Change stuffed status to false, continue walking.
walking(stuffedTimeout, StateData) ->                              
	put(stuffed,false),												% Switch to not stuffed.
    {next_state, walking, StateData};

%% Get escape event while the dragon is walking.
%% Change the dragon state to escaping.
walking({srv_escape, Hpos}, _StateData) ->                       	% Dragon is near a Whitewalker, into whitewalker_radius area.
	put(whitewalkerPos, Hpos),
	cancelCurrentEvent(),                                           % Cancel walking event.
	continue(?CLK, {escape, get(pos)}),                           	% Generate escaping event.
	{next_state, escaping, {true}};                                 
	
%% Get obstacle event while the dragon is walking.
...
%%							False: another dragon eat the relevant whiteking -> continue walking (change state to walking).
		...
		true ->
			...
			case distanceFrom(NextPos, Wpos) =< ?MINIMUM_RANGE/2 of
				true ->  												% Reaches whiteking
						...
						restartFoodTimer(?DRAGON_TIMEOUT_FOOD),
						continue(?DRAGON_EATING_TIMEOUT, finishEating),		% after eating_timeout send finishEating event.
						{next_state, eating, StateData};
				_Else->
						put(pos,NextPos),
						put(whitekingFlag, false),						% lock the whiteking from another dragon.
						sendPOS(NextPos, walking),					
...
%% finishEating event received while dragon is in eating state.
%% duplicate the dragon.
eating(finishEating, StateData) ->
	%square_server:addDragon(get(server), erlang:list_to_atom(erlang:ref_to_list(make_ref())),
		%get(pos), get(pos),?DRAGON_TIMEOUT_FOOD,false),
	io:fwrite("Dragon!!!!~n"),
	restartFoodTimer(?DRAGON_TIMEOUT_FOOD),
...
%% server escape event received while dragon is in eating state.
...
%% Obstacle event received while dragon is in eating state.	
...
%% The server will check radiuses and update the dragon state accordingly.
sendPOS(POS, StateName) ->
	case square_server:checkRadsAndUpdate(get(server),get(id), dragon, POS, StateName) of
...
updateEatenW(Wpos) ->
	...
	startStuffedTimer(?DRAGON_TIMEOUT_STUFFED).							% Start the stuffed timer
...
getCurrentSpeed(State) ->
			case State of
				escaping -> ?DRAGON_FAST_SPEED;
				_Else    -> ?DRAGON_SLOW_SPEED
			end.
...
%~~~~~~~~~~~~~~~~~~~~~~~~~ checkChangeServer ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%% Check if the dragon is outside it's server area. if so, sends the server "changeServer" msg. 
checkChangeServer(POS) ->
	case isOut(POS) of
		true  -> 
			gen_fsm:send_event_after(0, delete),						% Dragon kill itself.
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~getNextPos~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%% Returns the new position(X,Y) according to the dragon current position, destination and step type.
%% Dragon swin on a straight line.
%% Parameters: 
